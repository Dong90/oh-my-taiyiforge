...
describe("commands.yaml ↔ prompts 对账", () => {
  ...
  it(`canonical_v${CANONICAL_CATALOG_VERSION} 与 slash_catalog.recommended_v${CANONICAL_CATALOG_VERSION} 均为 21 条且一致`, () => {
    const sync = validateCanonicalCatalogSync(yaml, catalogSections);
    expect(sync.ok, sync.ok ? "" : sync.errors.join("\n")).toBe(true);
    const recKey = `recommended_v${CANONICAL_CATALOG_VERSION}` as keyof typeof catalogSections;
    expect(catalogSections[recKey]).toHaveLength(21);
  });

  it(`canonical_v${CANONICAL_CATALOG_VERSION} umbrella legacy_map 目标均有 prompt`, () => {
    const missing: string[] = [];
    for (const slash of parseCanonicalLegacyMapTargets(yaml)) {
  ...
  it(`canonical_v${CANONICAL_CATALOG_VERSION} token engine_map 均有 taiyi-token-* prompt（或 allowlist 标记为合并）`, () => {
    const missing: string[] = [];
    for (const key of parseCanonicalTokenEngineKeys(yaml)) {
  ...
  it("slash_catalog / engine_slash 每条斜杠均有对应 prompt 文件", () => {
    ...
    expect(slashes.length).toBeGreaterThan(40);
  ...
  it("每个 prompts/taiyi*.md（除 allowlist）在 commands.yaml 中有引用", () => {
    ...
    for (const file of prompts) {
      ...
      const stem = file.replace(/^taiyi-/, "").replace(/\.md$/, "");
      const inYaml =
        yaml.includes(`taiyi-${stem}`) ||
        yaml.includes(`/taiyi:${stem}`) ||
        yaml.includes(`$taiyi-${stem}`);
