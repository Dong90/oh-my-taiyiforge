...
describe("generateWiring", () => {
  const minimal: WiringScanResult = { routers: [{ variable: "router", modulePath: "app.controllers.v1.translation_controller", prefix: "/api/v1/translation", decorators: ["/translate", "/translate/stream"] }], middlewares: [], inits: [] };

  it("generates import with full-path alias to avoid collisions", () => {
    const code = generateWiring(minimal);
    expect(code).toContain("AUTO-GENERATED");
    expect(code).toContain("from app.controllers.v1.translation_controller import router as app_controllers_v1_translation_controller_router");
  ...
  it("generates middleware registrations in correct order", () => {
    const scan: WiringScanResult = { routers: [], middlewares: [{ className: "JWTAuthMiddleware", modulePath: "app.middleware.auth" }, { className: "PrometheusMetricsMiddleware", modulePath: "app.telemetry.metrics" }], inits: [] };
  ...
  it("generates configured middleware with production warning for known classes", () => {
    const scan: WiringScanResult = { routers: [], middlewares: [{ className: "CORSMiddleware", modulePath: "fastapi.middleware.cors" }], inits: [] };
    const code = generateWiring(scan);
  ...
  it("generates init function calls for setup-style inits", () => {
    const scan: WiringScanResult = { routers: [], middlewares: [], inits: [{ funcName: "setup_tracing", modulePath: "app.telemetry.tracing", callStyle: "direct" }] };
    const code = generateWiring(scan);
  ...
  it("generates lifespan wrapper for generator-style inits (get_db)", () => {
    const scan: WiringScanResult = { routers: [], middlewares: [], inits: [{ funcName: "get_db", modulePath: "app.db.engine", callStyle: "yield" }] };
    const code = generateWiring(scan);
  ...
  it("generates nested lifespan for multiple generator inits", () => {
    const scan: WiringScanResult = { routers: [], middlewares: [], inits: [{ funcName: "get_db", modulePath: "app.db.engine", callStyle: "yield" }, { funcName: "get_cache", modulePath: "app.cache.redis_cache", callStyle: "yield" }] };
    const code = generateWiring(scan);
  ...
  it("full wiring combines routers + middlewares + inits", () => {
    const scan: WiringScanResult = { routers: [{ variable: "router", modulePath: "app.controllers.v1.translation_controller", decorators: ["/translate"] }, { variable: "router", modulePath: "app.controllers.auth_controller", decorators: ["/login"] }], middlewares: [{ className: "PrometheusMetricsMiddleware", modulePath: "app.telemetry.metrics" }], inits: [{ funcName: "setup_tracing", modulePath: "app.telemetry.tracing", callStyle: "direct" }, { funcName: "get_db", modulePath: "app.db.engine", callStyle: "yield" }] };
    const code = generateWiring(scan);
    expect(code).toContain("AUTO-GENERATED");
