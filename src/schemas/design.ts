...
export const DesignSchema = z.object({
  ...
  ci_cd: z.string().optional().describe("CI/CD 配置变更（如 workflow / deploy / npm publish）"),
  rollback_trigger: z.string().optional().describe("回滚条件"),
  rollout_steps: z.array(z.string()).optional().describe("上线步骤（灰度/发布顺序）"),
  security_threats: z.array(SecurityThreat).optional().describe("安全威胁建模"),
  data_model: z.string().optional().describe("数据模型 DDL（无则填 '无数据模型变更'）"),
  api_changes: z.string().optional().describe("API 变更描述（无则填 '无 API 变更'）"),
  key_flow: z.string().optional().describe("关键流程 Mermaid 时序图（无则填 '无关键流程变更'）"),
