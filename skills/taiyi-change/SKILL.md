...
- `npx taiyi guide <slug>` 的 `recommendedAuxiliary`、`complexity`
...
1. `npx taiyi next <slug>` — 看推荐辅助 Skill（通常含 `taiyi-intel-scan`）
...
1. 对照质量自检清单
2. `npx taiyi complete <slug> change`（**人工门**：change 默认需审批）
