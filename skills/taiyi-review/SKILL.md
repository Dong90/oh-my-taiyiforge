...
用户打 **`/taiyi:review-loop`** = **任意阶段可直接启动** review 会话循环：Agent **在同一会话内**重复审查，**机器门禁通过前不得停止**（对齐 `/taiyi:loop`）。

```bash
scripts/taiyi-forge.sh review-loop <slug>   # 启动循环（强制先新一轮 review，不直接过关）
scripts/taiyi-forge.sh review-check <slug>  # 写完 REVIEW.md 后跑机器门禁
```
...
`scripts/taiyi-forge.sh complete <slug> review` 或 `npx taiyi complete <slug> review --approver "审批人"`（OpenCode `taiyi_complete` 须传 `approver`）
