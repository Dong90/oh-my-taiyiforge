...
- **入口函数**（被 CLI/HTTP/定时触发）→ 必须有外部调用者
- **中间函数**（被入口函数内部调用）→ 必须在 diff 中有调用者
- **独立函数**（无调用者）→ 标记为 dead code 或标注依赖 change

检查方式：grep 新增函数名在整个 codebase 中的调用次数。
- 0 次调用 → **critical finding**：函数定义但无调用者
- 同步检查 task.json 的 `covers_frs` 与 requirement.json 的 `functional_requirements` 覆盖是否完整
...
- **export 调用者检查**: git diff 中所有新增/修改 export，grep 查找调用者。零调用者 → 阻止过关
- **上游工件完整性**: 所有非跳过阶段工件必须存在且非种子模板 → 阻止过关
- **分数门控**: 代码/文档/测试/总评 四项分数不达标 → 阻止过关（TAIYI_REVIEW_ENFORCE_SCORES=0 关闭）
- **轮次门控**: review-loop 必须完成全部轮次 → 阻止过关
- **TDD 证据**: TEST.md 不能只写 "build passes" 而无实际 .test.ts 文件引用
