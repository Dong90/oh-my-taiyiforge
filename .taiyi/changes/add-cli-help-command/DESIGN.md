...
# DESIGN: CLI --help design

> **一句话**: commander.js built-in help
...
- **选定**: commander.js built-in help
  理由: 项目已用 commander, 零新增依赖
- **部署**: npm package
- **约束**: TypeScript strict
...
CLI 无 help 子命令，用户靠 README 了解命令
...
|------|---------|------|------------|:-------:|:--------:|
| commander | `^13` | CLI 框架 |  | `13.1.0` | 已检查 |
...
| 模块 | 操作 | 路径 | 说明 |
|------|------|------|------|
| help.ts | 新增 | packages/cli/src/commands/ | help 命令 |
| index.ts | 修改 | packages/cli/src/ | 注册 help 命令 |
...
- `packages/cli/src/commands/help.ts`（新增）
**禁动清单**:
- `packages/orchestrator`（AI 不许碰）
...
| 方案 | 名称 | 思路 | 优点 | 缺点 | 代价 |
|------|------|------|------|------|------|
| A | commander.js built-in help | 使用 commander.helpInformation() + command.configureHelp() | 零依赖<br>与现有 CLI 一致<br> |  | 低 |
| B | 手写 help 函数 | 自建 help.ts 硬编码命令列表 | 完全自定义<br> | 增加维护负担<br>命令变更时需手动同步<br> | 中 |
...
- **Chosen:** A
- **Reason:** 零维护成本, 与 commander.js 深度集成
- **取舍:** 样式受 commander 限制，但满足需求
...
| 决策 | 半径 | 最坏情况 | 隔离 |
|------|:--:|---------|------|
| 使用 commander built-in help | 低 | help 输出格式不符预期 | 仅影响 help 命令本身 |
...
| 决策 | Token? | 不选成熟方案的理由 |
|-----|:--:|-------------------|
| commander built-in help | 否 | 成熟框架能力 |
...
| 威胁 | 攻击向量 | 缓解 |
|------|---------|------|
| 信息泄露 | help 输出暴露内部路径 | 使用 commander 标准输出 |
...
- [reusable-abstraction] help 格式化逻辑可提取为独立模块供其他 CLI 复用
